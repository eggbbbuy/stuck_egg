...
STOCKS = [
    ...
    # 封裝測試
    ("3711", "日月光投控", "封測龍頭：AI 晶片先進封裝 / CoWoS 外溢 / 打線封裝"),
    ("2449", "京元電", "測試專業：AI 晶片 / CPU / 邏輯 IC 晶圓・成品測試"),
    ("6239", "力成", "封測：記憶體(DRAM/Flash) / 邏輯 IC + 先進封裝"),
    ("3374", "精材", "封裝：晶圓級封裝 WLP（CIS 影像感測 / MEMS）"),
    ("6271", "同欣電", "封裝：功率模組 / 車用功率封裝 + CIS 影像感測（混合 IC）"),
    ("2329", "華泰", "封測：分離式 / 功率元件（打線封裝）"),
    ("2369", "菱生", "封測：分離式 / 功率 / LED 封裝"),
    ("2441", "超豐", "封裝：打線封裝（功率分離式 / 邏輯 IC）"),
    ("6147", "頎邦", "封測：驅動 IC（COF / COG・面板驅動）"),
    ("3264", "欣銓", "測試：功率 / 類比 / 邏輯 IC 晶圓測試"),
    ("8150", "南茂", "封測：記憶體 / 驅動 IC"),
    ("6525", "捷敏-KY", "封測：功率半導體專門（MOSFET/IGBT/二極體/模組・全球前三大）"),
    # PMIC / 電源管理 IC
...
ADV_PKG = {"3711", "2449", "6239", "3374",        # 先進封裝/測試
           "6271", "2329", "2369", "2441", "6147", "3264", "8150", "6525"}  # + 功率/分離式 & 驅動IC 封測
...
def main():
    for code, name, sub in STOCKS:
        if code in CARRIER_PCB:
            ...
            group = "封裝測試"
            theme = "AI 先進封裝（CoWoS 外溢）+ 功率/分離式封測 + IC 測試放量"
        elif "被動" in sub:
            group = "被動元件"
